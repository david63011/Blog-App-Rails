...
RSpec.describe 'Users', type: :request do
  ...
  it 'is not valid with a name that is not a string' do
    subject.name = 1
    expect(subject).to_not be_valid
  end

  it 'is not valid with a bio that is not a string' do
    subject.bio = 1
    expect(subject).to_not be_valid
  end

  it 'post counter method should be 6'
  expect(subject.posts_counter).to eq(6)
end
